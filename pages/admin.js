...
export default function Admin() {
  ...
  return (
    <main style={{ maxWidth: 1200, margin: "40px auto", fontFamily: "system-ui" }}>
      ...
      {data && (
        <>
          <p style={{ color: "#666" }}>
            Stand: <b>{data.today}</b> • Schüler: <b>{rows.length}</b> • Neue/Tag Ziel: <b>{data.newPerDay}</b>
          </p>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1100 }}>
              <thead>
                <tr style={{ background: "#fafafa" }}>
                  ...
                  <th style={th}>Neu heute</th>
                  <th style={th}>Neu übrig</th>
                ...
                {rows.map((r) => {
                  ...
                  return (
                    <tr key={s.auth_user_id}>
                      ...
                      <td style={td}><b>{r.newToday ?? 0}</b></td>
                      <td style={td}>{r.newRemainingToday ?? 0}</td>

                      <td style={td}>
                        <b>{r.progressPct ?? 0}%</b>{" "}
                        <span style={{ color: "#777" }}>
                          ({r.mastered ?? 0}/{r.totalCards ?? 0})
                        </span>
                      </td>

                      <td style={td}><b>{r.dueNow ?? 0}</b></td>
                      <td style={td}>{r.totalCards ?? 0}</td>
            ...
            return (
              <div key={s.auth_user_id} style={{ border: "1px solid #eee", borderRadius: 10, padding: 12, marginTop: 12 }}>
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
                  ...
                  <div style={{ color: "#666" }}>
                    Neu heute: <b>{r.newToday ?? 0}</b> • Neu übrig: <b>{r.newRemainingToday ?? 0}</b> • Fortschritt: <b>{r.progressPct ?? 0}%</b>
                  </div>
                </div>
...
function MiniBars({ counts }) {
  ...
  const parts = [
    { v: counts?.[0] || 0, bg: "#f8d7da" },
    { v: counts?.[1] || 0, bg: "#fff3cd" },
    { v: counts?.[2] || 0, bg: "#d1ecf1" },
    { v: counts?.[3] || 0, bg: "#d4edda" },
    { v: counts?.[4] || 0, bg: "#c3e6cb" },
  ];
  ...
  return (
    <div style={{ display: "flex", height: 12, borderRadius: 999, overflow: "hidden", background: "#eee", minWidth: 180 }}>
      {parts.map((p, i) => (
        <div
          key={i}
          style={{ width: `${Math.round((p.v / total) * 100)}%`, background: p.bg }}
